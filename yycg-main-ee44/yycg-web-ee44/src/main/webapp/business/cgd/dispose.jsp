...
var columns = [ [ {
	...
	width : 180,
	formatter: function(value,row,index){
		var string= '<select name="yycgdmxList['+index+'].sysDictInfoCgzt.id">'
		+'<option value=""></option>'
		<c:forEach items="${yycgdCgZtList }" var="dictinfo">
		...
		$('#yycgdmxlist').datagrid({
			...
			url : '${baseurl}/cgd/yycgdmx_result.action',//这里后边带了一个参数，所以form中不需要此参数yycgdid
			 queryParams:{//url的参数，初始加载datagrid时使用的参数
				'yycgdCustom.id':'${yycgd.id}'//yycgdid是参数名称，如果参数名称中间有点，将参数用单引号括起来
			}, 
			idField : 'id',//采购药品明细id
