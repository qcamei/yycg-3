						...
						<TBODY>
							...
							<TR>
								...
								<TD class=category width="35%">
									...
									<c:forEach items="${useredit}" var="dictinfo">
										<option value="" name="${dictinfo.id }">${dictinfo.mc }</option>
										</c:forEach>
									</select>
								</TD>
								...
								<TD class=category width="35%">
									<c:forEach items="${userStateList}" var="dictinfo">
									   <input type="radio" id="selectType"name="sysUserCustom.sysDictInfoByUserstate.id"  value="${dictinfo.id}" />${dictinfo.info}
