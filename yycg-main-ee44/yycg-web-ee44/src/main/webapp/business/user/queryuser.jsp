...
$(function(){
	...
	var col_arr = [[
					...
					//方式1：通过指定field获得对应对象
					{field:'sysDictInfoByGroupid',title:'用户类别',width:80,
					...
					{field:'sysDictInfoByGroupid',title:'所属单位',width:180,
						formatter:function(value,row){
							//1 业务码
						...
						{field:'edit',title:'功能',width:180,width:80,formatter: 
							function(value,rowData,rowIndex){
							
							return "<a  class='editcls' onclick=\"edituser('"+rowData.id+"')\">修改</a>" }
						}, 
						{field:'delete',title:'删除',width:180,width:80,formatter: 
							function(value,rowData,rowIndex){
							
							return "<a  class='delcls' onclick=\"deluser('"+rowData.id+"')\">删除</a>" }
						}, 
			...
			var options={
					...
					onLoadSuccess:function(data){
						...
						$(".delcls").linkbutton({plain:true,iconCls:'icon-cancel'});
...
function deluser(userId){
		...
		_confirm("您确定要删除吗？",function(){
			...
			//然后ajax提交表单
			jquerySubByFId("sysuserdelForm",sysuserdel_callback);
		});
...
<form id="sysuserdelForm" action="${baseurl }/user/deleteuser_submit.action" method="post">
	<!-- form中包括要删除用户的id -->
	<input type="text"  id="sysuserdelid" name="sysUserCustom.id"/>
</form>
</body>
