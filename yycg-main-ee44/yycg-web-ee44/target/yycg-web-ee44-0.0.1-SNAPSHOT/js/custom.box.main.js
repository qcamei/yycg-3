 ...
 * @param data  显示数据
 * @param fn 成功回调函数
 * @param errFn  错误的回调函数（暂时没有）
 */
function message_alert(data ,fn , errFn){
	...
	//提示信息回调函数获得服务器返回的额外数据
	var sysdata = data_v.sysdata;
	sysdata = sysdata || {};
	
	//alert(message);
	if(type==0){
		$.messager.alert('提示信息',message,'error',function(){errFn( ); });
	}else if(type==1){
		$.messager.alert('提示信息',message,'info',function(){fn( sysdata ); });
	}else if(type==2){
		$.messager.alert('提示信息',message,'warning',function(){ errFn(); });
	}else if(type==3){
